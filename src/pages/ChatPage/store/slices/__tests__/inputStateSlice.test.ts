...
describe("inputStateSlice", () => {
  ...
  it("does not update pendingQuestionRespond when the payload is unchanged", () => {
    const harness = createSliceHarness<InputStateSlice>(createInputStateSlice as any);
    ...
    const first = harness.getState().pendingQuestionRespond;
    ...
    expect(harness.getState().pendingQuestionRespond).toBe(first);
  });
